...
/*  Adds the question ID and Type (Multi choice or Programming) to the index 
 *  of qnum for an individual student specified by student_name to their hashtable entry
 *  Used when a question has been generated by the QB for a specific student
 */
...
/*  Pings the Question Bank specified at socket
 *  The QB must have already been connected and allocated
 *  a spot in the array of qb_info (the QB's index inside
 *  the array is qb_num)
 *  If the QB fails to respond, its spot in qb_info opens
 *  up for a new QB to (re)connect
 */
...
/*  Assigns a connected QB socket to the array of accepted
 *  QBs in the qb_array
 *  First checks that there is a spot in the array to house
 *  the QB connection - if not return -1
 *  If space available, QB is assigned a spot
 *  Type is the type of questions the QB has (C or PYTHON)
 */
...
/*  Queries the QBs for the string corresponding to qid
 *  Returns that string
 *  QID is odd if its a Python question, even if C
 *  so it uses this knowledge to work out which QB to ask
 */
char *get_question(int qid)
{
    ...
    // Handle Response - strtok twice to get question from QB response protocol
...
/* MAIN FUNCTION OF TM
 * Takes one argument - port number to host on
 */
int main(int argc, char *argv[])
{
    ...
    for (int i = 0; i < NUM_QB; i++)
    {
        // sets the qb info to blank to allow new connections
        qb_info[i].socket = 0;
        qb_info[i].type = NONE;
    }
    ...
    manage_connection(socket); // initial setup now finished
    return 0;
}
